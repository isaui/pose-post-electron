...
// Import Prisma and photo queue routes
const { PrismaClient } = require('./generated/prisma');
const photoQueueRoutes = require('./routes/photoQueue');
...
// Register photo queue routes
app.use('/api/photo-queue', photoQueueRoutes);
...
async function runCameraService() {
  try {
    ...
    console.log('Starting live view automatically...');
    setupLiveView();
...
app.post('/api/print', async (req, res) => {
  try {
    ...
    const printCommand = path.join(process.cwd(), 'executables', 'PDFtoPrinter.exe');
...
server.listen(port, '0.0.0.0', () => {
  main();
  // Jalankan queue processor dengan worker threads
